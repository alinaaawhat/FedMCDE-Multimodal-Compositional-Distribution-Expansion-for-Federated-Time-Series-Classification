...
class PatternIdentifier(nn.Module):
    def __init__(self, feat_dim, num_patterns):
        super().__init__()
        self.prototypes = nn.Parameter(torch.randn(num_patterns, feat_dim))

    def forward(self, feats):
        # feats: [B, D]
        logits = torch.matmul(feats, self.prototypes.t())  # [B, M]
...
class PatternExperts(nn.Module):
    ...
    def forward(self, feats, p):
        # feats: [B, D], p: [B, M]
        expert_outs = torch.stack(
            [expert(feats) for expert in self.experts], dim=1
        )  # [B, M, D]

        p = p.unsqueeze(-1)  # [B, M, 1]
        fused = (expert_outs * p).sum(dim=1)  # [B, D]
        return fused


class PatternConditioner(nn.Module):
    def __init__(self, feat_dim, num_patterns):
        super().__init__()
        self.PI = PatternIdentifier(feat_dim, num_patterns)
        self.MoE = PatternExperts(feat_dim, num_patterns)

    def forward(self, feats):
        p = self.PI(feats)
        feats_hat = self.MoE(feats, p)
        return feats_hat, p
...
def Trainer_ft(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, train_dl, valid_dl, test_dl, device, logger, config, experiment_log_dir, training_mode,testuser = None):
    ...
    # Initialize Pattern Conditioner
    pattern_conditioner = PatternConditioner(
        feat_dim=config.TC.hidden_dim,
        num_patterns=getattr(config, 'num_patterns', 8)
    ).to(device)
    pattern_optimizer = torch.optim.Adam(pattern_conditioner.parameters(), lr=config.lr)
    ...
    for epoch in range(1, max_epochs):
        # Train and validate
        train_loss, train_acc = model_train(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, criterion, train_dl, config, device, training_mode, pattern_conditioner, pattern_optimizer)
        valid_loss, valid_acc, _, _ = model_evaluate(model, temporal_contr_model, valid_dl, device, training_mode)
        if training_mode != 'self_supervised':  # use scheduler in all other modes.
            scheduler.step(valid_loss)
        if valid_acc > best_acc:
            best_acc = valid_acc
            chkpoint = {'model_state_dict': model.state_dict(), 'temporal_contr_model_state_dict': temporal_contr_model.state_dict(), 'pattern_conditioner_state_dict': pattern_conditioner.state_dict()}
...
def model_train(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, criterion, train_loader, config, device, training_mode):
    ...
    for batch_idx, minibatch in enumerate(train_loader): # len(minibatch)=3, the last element indicates the type of the original data
        ...
        if training_mode == "self_supervised":
            lambda1 = 1
            lambda2 = 0.7 # 0.4 0.5 0.6 0.7 0.8
